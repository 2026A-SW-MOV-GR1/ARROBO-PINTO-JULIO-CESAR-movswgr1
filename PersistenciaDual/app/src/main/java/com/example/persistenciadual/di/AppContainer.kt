...
class AppContainer(context: Context) {
    ...
    private val sqliteHelper = TaskSqliteHelper(context)

    private val sqliteRepository = SqliteTaskRepository(
        helper = sqliteHelper,
        logger = logger,
        applicationScope = applicationScope
    )

    private val jsonRepository = JsonTaskRepository(
        context = context,
        logger = logger,
        applicationScope = applicationScope
    )

    val dualTaskRepository = DualTaskRepository(
